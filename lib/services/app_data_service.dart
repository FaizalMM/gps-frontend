...
class AppDataService {
  ...
  void Function(BusModel? bus)? _onHomeUpdate;

  /// Mulai polling status bus untuk Home tab siswa.
  /// Update busesStream agar StreamBuilder di Home langsung reaktif.
  /// Interval 5 detik. Panggil dari _SiswaHomeTabState.initState().
  void startHomePolling({void Function(BusModel? bus)? onUpdate}) {
    if (_homePollingActive) return;
    _homePollingActive = true;
    _onHomeUpdate = onUpdate;
    _pollHomeBus(); // langsung poll sekali
    _homePollingTimer = Timer.periodic(const Duration(seconds: 5), (_) {
      if (_homePollingActive) _pollHomeBus();
    });
  }

  /// Stop home polling. Panggil di dispose _SiswaHomeTabState.
  void stopHomePolling() {
    _homePollingActive = false;
    _homePollingTimer?.cancel();
    _homePollingTimer = null;
    _onHomeUpdate = null;
  }

  Future<void> _pollHomeBus() async {
    if (!_homePollingActive) return;
    try {
      final hasToken = await _busService.hasValidToken();
      if (!hasToken) {
      ...
      // Panggil callback langsung ke Home tab state — cara paling pasti
      _onHomeUpdate?.call(bus);
      ...
      if (idx >= 0) {
        ...
        if (changed) {
          existing.updateGps(
            ...
            speed: bus.gpsActive ? bus.speed : 0,
            gpsActive: bus.gpsActive,
          );
